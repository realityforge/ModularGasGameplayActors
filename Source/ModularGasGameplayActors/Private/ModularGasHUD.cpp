 ...
 */
#include "ModularGasHUD.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystemGlobals.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(ModularGasHUD)

void AModularGasHUD::GetDebugActorList(TArray<AActor*>& InOutList)
{
    ...
    {
        if (const auto AbilitySystemComponent = *It)
        {
            if (!AbilitySystemComponent->HasAnyFlags(RF_ClassDefaultObject | RF_ArchetypeObject))
            {
                ...
                {
                    const auto OwnerActor = AbilitySystemComponent->GetOwnerActor();
